...
const App = () => {
  ...
  useEffect(() => {
    ...
    socket.on("emergencyRequest", (data) => {
      ...
      audio
        .play()
        .catch(() => console.log("Audio file missing, skipping sound."));

      // Show Alert Popup
      toast.custom(
        (t) => (
          <div
            className={`${t.visible ? "animate-enter" : "animate-leave"} max-w-md w-full bg-white shadow-2xl rounded-lg pointer-events-auto flex ring-1 ring-black ring-opacity-5 border-l-4 border-red-600`}
          >
    ...
    <div>
      ...
      <Routes>
        ...
        <Route
          ...
          element={
            !authUser ? <CollegeRegistrationPage /> : <Navigate to="/college" />
          }
        />
        <Route
          path="/college/login"
          element={
            !authUser ? <CollegeLoginPage /> : <Navigate to="/college" />
          }
        />
        <Route
          path="/college"
